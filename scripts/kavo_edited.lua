-- // ALL CREDITS GO TOWARDS THE MAKERS OF KAVO \\--
local Kavo = {}
...
function writefileifnotexists(path, text)
    if not isfile(path) then
        writefile(path, text)
    end
end

gameconfigpath = "NexusExploits/Games/" .. tostring(game.PlaceId) .. ".json"

createfolderifnotexists("NexusExploits")
createfolderifnotexists("NexusExploits/Games/")
...
writefileifnotexists(gameconfigpath, game:GetService("HttpService"):JSONEncode(default))

local config = game:GetService("HttpService"):JSONDecode(readfile(gameconfigpath)) or default

function settoggle(key, value)
    table.foreach(config, print)
    table.foreach(config.toggles, print)
        if value == true then
        config.toggles[key] = true
        writefile(gameconfigpath, game:GetService("HttpService"):JSONEncode(config))
    end
    if value == false then
        -- config.toggles[key] = false
        -- can we remove the key?
        -- table.remove takes a number as a second argument, which is the index of the key to remove
        -- but we don't know the index of the key, so we can't use it
        -- so we have to loop through the table and find the key we want to remove

        config.toggles[key] = nil
...
Kavo.SliderValueAlwaysShown = false

function Kavo:SetSliderValueAlwaysShown(b)
    Kavo.SliderValueAlwaysShown = b
end

function Kavo:GetSlider(name)
    return config.sliders[name] or nil
end

function Kavo:GetToggle(name)
    return config.toggles[name] or nil
end
    ...
    function Tabs:NewTab(tabName)
        ...
        function Sections:NewSection(secName, hidden)
            ...
            function Elements:NewToggle(tname, nTip, callback)
                ...
                local toggled = config.toggles[tname] or false
                ...
                if toggled then
                    ...
                    toggleEnabled.Visible = true
                    toggleEnabled.ImageTransparency = 0
                end

                btn.MouseButton1Click:Connect(function()
                    settoggle(tname, not toggled)
                    if not focusing then
            ...
            function Elements:NewSlider(slidInf, slidTip, maxvalue, minvalue, callback)
                ...
                startVal = startVal or config.sliders[slidInf] or 0
                callback = callback or function()
                end
                ...
                if config.sliders[slidInf] then
                    sliderDrag.Size = UDim2.new(0, config.sliders[slidInf] * (149 / maxvalue), 1, 0)
                end
                ...
                val.TextSize = 14.000
                if not Kavo.SliderValueAlwaysShown then
                    ...
                    if not focusing then
                        ...
                        releaseconnection = uis.InputEnded:Connect(function(Mouse)
                            if Mouse.UserInputType == Enum.UserInputType.MouseButton1 then
                                ...
                                val.Text = Value
                                setslider(slidInf, Value)
                                -- check for SliderValueAlwaysShown, should we use Kavo or self? we should use Kavo.
                                if not Kavo.SliderValueAlwaysShown then
