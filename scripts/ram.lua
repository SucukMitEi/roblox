...
function Account:GetField(Field)
    return GET('GetField', self.Username, 'Field=' .. Field)
end
function Account:SetField(Field, Value)
    return GET('SetField', self.Username, 'Field=' .. Field, 'Value=' .. tostring(Value))
end
function Account:RemoveField(Field)
    return GET('RemoveField', self.Username, 'Field=' .. Field)
end
...
function Account:LaunchAccount(PlaceId, JobId, FollowUser, JoinVip) -- if you want to follow someone, PlaceId must be their user id
    return GET('LaunchAccount', self.Username, 'PlaceId=' .. PlaceId, JobId and ('JobId=' .. JobId),
        FollowUser and 'FollowUser=true', JoinVip and 'JoinVIP=true')
end
